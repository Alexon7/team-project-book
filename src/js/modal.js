...
export const modal = async content => {
  const modal = basicLightbox.create(content, {
    onShow: instance => {
      ...
      const btnAddBookToShoppingList = instance.element().querySelector('.btnAdd');
      const btnRemoveBookFromShoppingList = instance.element().querySelector('.btnRemove');

      btnAddBookToShoppingList.addEventListener('click', addBookToShoppingList);
      btnRemoveBookFromShoppingList.addEventListener('click', removeBookFromShoppingList);
    },
    onClose: () => {
